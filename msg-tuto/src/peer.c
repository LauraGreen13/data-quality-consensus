...
void validate_received_data(peer_t peer, task_data data) {
  ...
  peer->my_data_length = peer->my_data_length + 1;
  //int *res = peer->my_data;
  
  for (i=0; i < peer->my_data_length; i++) {
    XBT_INFO("My_data for peer %s is :%d",peer->me, peer->my_data[i]);
  }
}
...
msg_error_t peer_wait_for_message(peer_t peer)
{
  ...
  while (!done) {
    ...
    if ((idx = MSG_comm_waitany(peer->pending_recvs)) != -1) {
      ...
      XBT_INFO("Received data is : %d", data->value);

      validate_received_data(peer, data);
      ...
      if (peer->next != NULL) {
        XBT_INFO("Sending data %d  from peer %s to peer %s ", data->value, peer->me, peer->next);
        peer_forward_msg(peer->me, data);
      }
...
{
  p->init = 0;
  p->my_data = malloc(100*sizeof(int));
  p->my_data_length = 0;
  p->prev = NULL;
  p->next = NULL;
  p->pieces = 0;
